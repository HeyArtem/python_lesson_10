...
if __name__ == "__main__": # Что это???
   ...
   #'''Здесь мы указываем, сколько можно сделать попыток игроку "Игрок-1".И если это количество будет больше разрешенного, то будкт выводиться сообщение "Игра закончена" '''
    for i in range(2):
        try:
